                      ...
                      tabItems(tabItem('subirdata',
                               ...
                               tabItem('corrplots',
                                       box(h1('GRÁFICO CORRELACIONES'),background = 'purple', width = 1000),
                                             d3heatmapOutput('plotcorr', width = 1100, height = 600),
                               ...
                               tabItem('calormap',
                                       ...
                                       d3heatmapOutput('HEATMAP',width = 1100, height = 600),
                                       h2('Base de Datos'),
                               ...
                               tabItem('numclusters',
                                       box(h3('Número de clusters sugeridos por método silhouette (for average silhouette width)'),background = 'purple', width = 250),
                                       plotOutput('silhouette'),
                                       h3('Número de clusters sugeridos por método wss (for total within sum of square)'),
                                       plotOutput('wss')),
                               ...
                               tabItem('model1', ##Ingreso de grafica y cuadors de la prediccion para el modelo 1
                                       ...
                                       box(
                                         title = 'FPS', 
                                         tableOutput('FPSUVB'), width = 4,height = 2,
                                         solidHeader = T, collapsible = T, 
                                         status = 'primary'),
                                       box(
                                         title = 'FPUVA',
                                         tableOutput('FPSUVA'), width = 4,height = 2,
                                         solidHeader = T, collapsible = T, 
                                         status = 'primary'),
                                       box(
                                         title = 'LONGITUD DE ONDA CRÍTICA',
                                         tableOutput('longcrit'), width = 4, height = 2,
                                         solidHeader = T, collapsible = T, 
                                         status = 'primary'),
                                       br(),#### Incluir las cuadros de los precions y calculo del mod1
                                       br(),
                                       br(),
                                       br(),
                                       br(),
                                       ...
                                       box(title = 'Costos Simulador por Kg',
                                           tableOutput('basessimulamod1'), width = 3, background = 'blue', collapsible = T),
                                       box(title = 'Costos filtros por Kg de formulación',
                                           DT::dataTableOutput('basesmod1'), collapsible = T)
                                       ),
                               tabItem('model2',
                                       box(
                                       ...
                                       box(
                                         title = 'FPS',
                                         tableOutput('FPSUVB2'), width = 4,height = 2,
                                         solidHeader = T, collapsible = T, 
                                         status = 'primary'),
                                       box(
                                         title = 'FPUVA',
                                         tableOutput('FPSUVA2'), width = 4,height = 2,
                                         solidHeader = T, collapsible = T, 
                                         status = 'primary'),
                                       box(
                                         title = 'LONGITUD DE ONDA CRÍTICA',
                                         tableOutput('longcrit2'), width = 4,height = 2,
                                         solidHeader = T, collapsible = T, 
                                         status = 'primary'),
                                       ...
                                       br(),
                                       box(title = 'Costos Simulador por Kg',
                                           tableOutput('basessimulamod2'), width = 3, background = 'blue', collapsible = T),
                                       box(title = 'Costos filtros por Kg de formulación',
...
output$lineas <- renderPlotly({
  ...
  bases_long2 <- gather(bases22, materia_prima, cantidad, X10000196:X10003660, factor_key = T)
  
  bases$clus <- factor(bases$clus)
  bases_long <- gather(bases, materia_prima, cantidad, X10000196:X10003660, factor_key = T)
 ...
 p<- ggplot(bases_long, aes(as.factor(x=materia_prima), y = cantidad, group = clus, 
    ...
    stat_summary(geom = 'line')+
    geom_point()+
    # geom_point(aes(group = bases_long$X))+
    theme(text = element_text(size = 14), legend.position = 'bottom', 
          axis.text.x = element_text(angle = 90, hjust = 1,size = 6))+
...
output$puntos <- renderPlotly({
    ...
    b<- ggplot(bases_long, aes(as.factor(x=materia_prima), y = cantidad, group = clus, 
                                 colour = clus,label = bases_long$materia_prima))+
      geom_point(aes(x=bases_long2$X, group = clus))+
      theme(text = element_text(size = 14), legend.position = 'bottom', # Cambiar tamaño del texto y la posición de la leyenda
            axis.text.x = element_text(angle = 90, hjust = 1, size = 6))+
...
output$FPSUVB <- renderValueBox({
  ...
  names(matriz)[3] <- "Abs"
  FPS <- round(mean(matriz$num)/mean((matriz$num*(10^(matriz$Abs*-1)))),0)
  valueBox(FPS, width = 3,
           subtitle = 'Predicción FPS modelo 1',
...
output$FPSUVA <-  renderValueBox({
  ...
  valueBox(FPFUVA, width = 3,
           subtitle = 'Predicción FPSUVA modelo 1',
...
valueBox(LOC, width = 3,
         subtitle = 'Longitud de onda crítica modelo 1',
...
output$basesmod1 <- DT::renderDataTable({
  ...
  LB_BASE_GOTERO_FPS25 <- c(7,7.2,30,2887,6397,9284)
  LB_CLARITE_ANTIMANC_FPS30 <- c(7.5,7.84,36,3093,6965,10059)
...
output$FPSUVB2 <- renderValueBox({
  ...
  valueBox(FPS, width = 3,
           subtitle = 'Predicción FPS modelo 2',
...
output$FPSUVA2 <-  renderValueBox({
  ...
  valueBox(FPFUVA, width = 3,
           subtitle = 'Predicción FPSUVA modelo 2',
...
output$longcrit2 <-  renderValueBox({
  ...
  valueBox(LOC, width = 3,
           subtitle = 'Longitud de onda crítica modelo 2',
...
output$basesmod2 <- DT::renderDataTable({
  ...
  nombres <- c('EMH', 'ZnO', 'In-vivo', 'Costo_EMH', 'Costo_ZnO', 'Costo_total')
  CY_STUDIO_LOOK_FPS20 <-  c(4, 2.21, 23, 1650,1963,3613)
  LB_CLARITÉ_ACLAR_MANC_FPS30 <- c(7,6.4,38,2887,5686,8573)
  CY_GO_NUDE_LIGERA_FPS20 <- c(5.96,6.03,34,2458,5357,7815)
  ESK_PRO_NATURAL_MATE_FPS20 <- c(5,4.2,29,2062,3731, 5794)
  MOD2bases <- data.frame(cbind(nombres,CY_STUDIO_LOOK_FPS20,LB_CLARITÉ_ACLAR_MANC_FPS30,
                                CY_GO_NUDE_LIGERA_FPS20,ESK_PRO_NATURAL_MATE_FPS20))
