...
# Se carga los datos para entrenar la red neuronal 
# Se carga los datos para predecir 
df <- read.xlsx("ArchivoDatosSimulador.xlsx", sheet = "filtrosabsmod1")
...
entradas <- data.frame(EMH = 7, ZnO =6)
...
plot(rnmod1)
set.seed(123)
rnmod1 <- neuralnet(a290+a291+a292+a293+a294+a295+a296+a297+a298+a299+a300+
...
###### Calculos UVA ########
## Se cargan los datos teoricos de PPD action spectrun * P(λ)*UVA radiation source
teoricoUVA <-read_xlsx("Libro2.xlsx", sheet = "uva")
...
matriz$pun_medio <- Reduce(function(a,b) 1*((a+b)/2), matriz$Abs, 
                           accumulate = T)
# Con los puntos medios calculados se calcula la suma acomulada que corresponde a la integral
matriz$integral <- Reduce(function(a,b) (a+b), matriz$pun_medio, 
                          accumulate = T)
...
datos290 <- df[1:3]
set.seed(123)
slit_data <- sample.split(df$EMH,SplitRatio = 0.8)
training_set <- subset(df, slit_data == TRUE)
test_set <- subset(df, slit_data == F)

### Entrenar la RNA con los datos de entrenamiento
set.seed(123)
rnmod1training_set <- neuralnet(a290+a291+a292+a293+a294+a295+a296+a297+a298+a299+a300+
                      a301+a302+a303+a304+a305+a306+a307+a308+a309+a310+a311+
                      a312+a313+a314+a315+a316+a317+a318+a319+a320+a321+a322+
                      a323+a324+a325+a326+a327+a328+a329+a330+a331+a332+a333+
                      a334+a335+a336+a337+a338+a339+a340+a341+a342+a343+
                      ...
                      a378+a379+a380+a381+a382+a383+a384+a385+a386+a387+a388+
                      a389+a390+a391+a392+a393+a394+a395+a396+a397+a398+a399+
                      a400~ EMH+ZnO, data = training_set, hidden = c(4,8,5,8),
                    threshold = 0.01)

## Prediccion del conjunto de datos de test set

presicciontest_set <- neuralnet::compute(rnmod1training_set,test_set)

predicciones <- data.frame(presicciontest_set$net.result)
view(data.frame(presicciontest_set$net.result))

# Hacer la transpuesta de los datos predichos y los datos de test para compararlos 
transpuestapredicciones <- t(predicciones)
transpuestatestset <- t(test_set[3:113])
comparacion <- cbind(transpuestatestset, transpuestapredicciones)

data.frame(table(transpuestatestset, transpuestapredicciones))




## K-folk cross validation 
kfold.crossvar.rna <- function(df,nfolds){
  fold <- sample(1:nfolds, nrow(df), replace = T)
  mean.sqr.error <- sapply(1:nfolds,
                            kfold.crossvar.rna.iter,
                            df, fold)
 list('MSE' = mean.sqr.error,
      'Overal_mean_sqr_erroe'= mean(mean.sqr.error),
      'sd_Mean_sqr_error'= sd(mean.sqr.error))                           
                           
}

kfold.crossvar.rna.iter<- function(k, df, fold){
  tr.id <- !fold %in% c(k)
  test.id <- fold %in% c(k)
  modrna <- neuralnet(a290+a291+a292+a293+a294+a295+a296+a297+a298+a299+a300+
                                    a301+a302+a303+a304+a305+a306+a307+a308+a309+a310+a311+
                                    a312+a313+a314+a315+a316+a317+a318+a319+a320+a321+a322+
                                    a323+a324+a325+a326+a327+a328+a329+a330+a331+a332+a333+
                                    a334+a335+a336+a337+a338+a339+a340+a341+a342+a343+
                                    ...
                                    a378+a379+a380+a381+a382+a383+a384+a385+a386+a387+a388+
                                    a389+a390+a391+a392+a393+a394+a395+a396+a397+a398+a399+
                                    a400~ EMH+ZnO, data = df[tr.id], hidden = c(4,8,5,8),
                                  threshold = 0.01) 
  pred <- neuralnet::compute(rnmod1training_set,df[test.id,])
  sqr.error <- (pred - df[test.id,'a290+a291+a292+a293+a294+a295+a296+a297+a298+a299+a300+
                                    a301+a302+a303+a304+a305+a306+a307+a308+a309+a310+a311+
                                    a312+a313+a314+a315+a316+a317+a318+a319+a320+a321+a322+
                                    a323+a324+a325+a326+a327+a328+a329+a330+a331+a332+a333+
                                    a334+a335+a336+a337+a338+a339+a340+a341+a342+a343+
